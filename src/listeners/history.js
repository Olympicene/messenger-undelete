...
for (var thread in config.allowed_threads) {
    ...
    fs.writeFile(databaseDir(config.allowed_threads[thread]), '[]', { flag: 'wx' }, (err) => {
        if (!err) console.log(`history-${config.allowed_threads[thread]}.json has been created`);
    });
}
    ...
    async listen(event) {
        if (this.type.indexOf(event.type) > -1) {
            fs.readFile(databaseDir(event.threadID), (err, data) => {
                ...
                if (json.length > 100) {json.pop()}
            
                fs.writeFile(databaseDir(event.threadID), JSON.stringify(json, null, '\t'), (err) => {
